        ...
        if request.device_type == PLATFORM_ANDROID and self.request.market != 'canto':
            for feed in feeds:
                res.append(dict(
                    ...
                    data=PostSerializer(feed.get_query().filter(legal=True).order_by('?')[:10], many=True, context={'caller': Post, 'request': request}).data
                    )
                )
            ...
            for genre in genres:
                res.append(
                    dict(
                        ...
                        data=PostSerializer(genre.post_set.all().filter(legal=True).order_by('?')[:10], many=True, context={'caller': Post, 'request': request}).data
                    )
                )

            conn().set(name='home_feed#' + PLATFORM_ANDROID, value=convert_to_dict(res), ex=86400)
        else:
            for feed in feeds:
                res.append(dict(
                    name=feed.name,
                    more=feed_link_pattern.format(reverse('songs:get-feed-list'), feed.code),
                    data=PostSerializer(feed.get_query().order_by('?')[:10], many=True,
                                        context={'caller': Post, 'request': request}).data
                )
                )
            ...
            for genre in genres:
                res.append(
                    dict(
                        name=genre.name,
                        more=feed_link_pattern.format(reverse('songs:get-genre-list'), genre.id),
                        data=PostSerializer(genre.post_set.all().order_by('?')[:10], many=True,
                                            context={'caller': Post, 'request': request}).data
                    )
                )
